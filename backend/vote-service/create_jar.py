...
if os.path.exists(manifest_path):
    with open(manifest_path, 'r') as f:
        content = f.read()
    # 替换 Start-Class 和其他字段
    content = content.replace("Start-Class: com.dzvote.activity.ActivityTestApplication", "Start-Class: com.dzvote.vote.VoteServiceApplication")
    content = content.replace("Implementation-Title: Activity Service", "Implementation-Title: Vote Service")
    with open(manifest_path, 'w') as f:
        f.write(content)
    print(f"已修改 MANIFEST.MF")
    print(f"Start-Class: com.dzvote.vote.VoteServiceApplication")
else:
...
if not os.path.exists(vote_classes_src):
    print(f"警告: vote-service 类文件不存在: {vote_classes_src}")
    print("请先编译 vote-service")
else:
