...
/**
 * 快速导出控制器 - 使用 JDBC 直接查询
 */
@RestController
@RequestMapping("/api/export/quick")
public class QuickExportController {

    private final JdbcTemplate jdbcTemplate;

    public QuickExportController(JdbcTemplate jdbcTemplate) {
    ...
    public void exportCandidates(@PathVariable Long activityId,
        ...
        try {
            // 使用 JDBC 原生查询，避免 MyBatis 类型处理器问题
            ...
            if (candidates.isEmpty()) {
                ...
                writer.print("{\"success\":false,\"message\":\"该活动没有候选人数据\"}");
                writer.flush();
                return;
            }
            ...
            try {
                response.setContentType("application/json;charset=UTF-8");
                PrintWriter writer = new PrintWriter(response.getWriter());
                writer.print("{\"success\":false,\"message\":\"导出失败: " + e.getMessage() + "\"}");
                writer.flush();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
    ...
    public void exportActivity(@PathVariable Long activityId,
        ...
        try {
            ...
            if (activities.isEmpty()) {
                response.setContentType("application/json;charset=UTF-8");
                PrintWriter writer = new PrintWriter(response.getWriter());
                writer.print("{\"success\":false,\"message\":\"活动不存在\"}");
                writer.flush();
                return;
            }
